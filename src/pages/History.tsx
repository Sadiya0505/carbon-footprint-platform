  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="flex items-center gap-3 mb-8">
        <HistoryIcon className="w-8 h-8 text-primary" aria-hidden="true" />
      ...
      <div className="bg-gradient-to-r from-orange-50 to-red-50 p-6 rounded-3xl mb-8 border border-orange-100 flex items-center justify-between shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-orange-900 mb-1">Current Streak</h2>
          <p className="text-orange-700">Keep calculating to build your habit!</p>
        </div>
        <div className="flex items-center gap-2 bg-white px-6 py-4 rounded-2xl shadow-sm border border-orange-100" aria-label={`Current streak: ${streak} days`}>
          <Flame className={`w-8 h-8 ${streak > 0 ? 'text-orange-500 fill-orange-500' : 'text-gray-600'}`} aria-hidden="true" />
      ...
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-900 mb-6">Emissions Over Time</h2>
        
        {history.length > 0 ? (
          <div className="h-80 w-full" role="img" aria-label="Line chart showing your carbon emissions over time">
          ...
          <div className="text-center py-12 text-gray-600" role="status">
            <p>No history yet. Calculate your footprint to start tracking!</p>
          </div>
        )}
      </div>
    </div>
